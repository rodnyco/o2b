<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('purchaser.dashboard')" :active="request()->routeIs('purchaser.dashboard')">
                {{ __('Моя доска') }}
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            <div class="px-4">
                <div class="font-medium text-base text-gray-800">{{ Auth::guard('purchaser')->user()->name }}</div>
                <div class="font-medium text-sm text-gray-500">{{ Auth::guard('purchaser')->user()->email }}</div>
            </div>
            ...
            <div class="mt-3 space-y-1">
                ...
                <form method="POST" action="{{ route('purchaser.logout') }}">
                    ...
                    <x-responsive-nav-link :href="route('purchaser.logout')"
                            onclick="event.preventDefault();
                                        this.closest('form').submit();">
                        {{ __('Выход') }}
                    </x-responsive-nav-link>
